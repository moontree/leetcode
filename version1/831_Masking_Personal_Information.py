...
and all letters between the first and last letter of
the first name must be replaced by 5 asterisks '*'.


2. Phone number:

A phone number is a string consisting of only the digits 0-9 or
...
class Solution(object):
    def maskPII(self, S):
        ...
        if '@' in S:
            name, keep = S.split('@')
            name_mask = name[0].lower() + '*****' + name[-1].lower()
            return name_mask + '@' + keep.lower()
        else:
            nums = '0123456789'
            digits = []
            for c in S:
            ...
            print digits
            if len(digits) == 10:
                return "***-***-%s" % ''.join(digits[-4:])
            else:
                return '+%s-***-***-%s' % ('*' * (len(digits) - 10), ''.join(digits[-4:]))
