...
we first jump to i = 1 because A[1] is the smallest value in (A[1], A[2], A[3], A[4])
that is greater than or equal to A[0].

During our 2nd jump (even numbered),
we jump from i = 1 to i = 2 because A[2] is the largest value in (A[2], A[3], A[4])
that is less than or equal to A[1].
...
class Solution(object):
    def oddEvenJumps(self, A):
        ...
        """
        n = len(A)
        pairs = [(v, i) for i, v in enumerate(A)]
        nxt_large = sorted(pairs, key=lambda x: [x[0], x[1]])
        nxt_small = sorted(pairs, key=lambda x: [-x[0], x[1]])
        nxt = [[-1, -1] for _ in range(n)]
        q = []
        for _, i in nxt_large:
            while q and q[-1] < i:
                nxt[q.pop()][0] = i
            q.append(i)
        while q:
        ...
        # print 'nxt large', [v[0] for v in nxt]
        # print 'nxt small', [v[1] for v in nxt]

        dp = [[0, 0] for _ in range(n)]
        dp[-1] = [1, 1]
        # dp[i][0]: odd jump can reach
        # dp[i][1]: even jump can reach
        ...
        for i in range(n):
            if dp[i][0] == 1:
                res += 1
        # print A
        # print [x[0] for x in dp]
        # print [x[1] for x in dp]
        return res
