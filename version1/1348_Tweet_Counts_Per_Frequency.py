"""
=========================
Project -> File: leetcode -> 1348_Tweet_Counts_Per_Frequency.py
Author: zhangchao
=========================
...
epresenting the time interval to get the total number of occurrences for the given tweetName.
The first time interval always starts from the startTime,
so the time intervals are [startTime, startTime + delta*1>,
...
class TweetCounts(object):
    ...
    def recordTweet(self, tweetName, time):
        ...
        bisect.insort_left(self.records[tweetName], time)
    ...
    def getTweetCountsPerFrequency(self, freq, tweetName, startTime, endTime):
        ...
        :rtype: List[int]
        """
        step = self.steps[freq]
        times = self.records.get(tweetName, [])
        res, tmp = [], 0
        for cur in range(startTime, endTime + 1, step):
            l, r = cur, min(cur + step - 1, endTime)
            li, ri = bisect.bisect_left(times, l), bisect.bisect_right(times, r)
            res.append(ri - li)
        return res
