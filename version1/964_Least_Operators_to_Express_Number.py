...
we will write an expression of the form x (op1) x (op2) x (op3) x ... where each operator op1, op2, etc.
    ...
    def leastOpsExpressTarget(self, x, target):
        ...
        for i in xrange(2, target + 1):
            ...
            for j in range(left, i):
                diff = i - j
                cache[i] = min(cache.get(i, n), cache.get(j, n) + cache.get(diff, n) + 1)

            for j in range(i + 1, min(right, n) + 1):
                diff = j - i
                cache[i] = min(cache.get(i, n), cache.get(j, n) + cache.get(diff, n) + 1)
