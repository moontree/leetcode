...
Each fork can be held by only one philosopher and so
a philosopher can use the fork only if it is not being used by another philosopher.
...
Design a discipline of behavior (a concurrent algorithm)
...
It is possible that the function will be called for the same philosopher more than once,
...
Output:
    [
    [4,2,1],[4,1,1],[0,1,1],[2,2,1],[2,1,1],[2,0,3],[2,1,2],[2,2,2],[4,0,3],[4,1,2],
    [0,2,1],[4,2,2],[3,2,1],[3,1,1],[0,0,3],[0,1,2],[0,2,2],[1,2,1],[1,1,1],[3,0,3],
...
class DiningPhilosophers(object):
    ...
    def __init__(self):
        self.persons = Semaphore(4)  # at most 4 person picked up forks
        self.locks = [Semaphore(1) for _ in range(5)]  # semaphore for each single fork
